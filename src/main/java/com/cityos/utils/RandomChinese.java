...
public class RandomChinese {
  ...
  /**
   * 获取一个长度为2的字节数组，共同组成一个GBK编码的汉字. <p>原理是从汉字区位码找到汉字。在汉字区位码中分高位与底位， 其中既有简体又有繁体。位数越前生成的汉字繁体的机率越大。<br>
  ...
  private static byte[] nextCharArray() {
    ...
    array[0] = (new Integer(highPas).byteValue());
    array[1] = (new Integer(lowPos).byteValue());
    // 返回字符数组的第一个字符
    return array;
  }
  ...
  public static char nextChar() throws Exception {
    // 将两个字节按照UTF-8方式解析成字符串，然后将字符串转换成字节数组
    char[] ret = new String(nextCharArray(), "GBK").toCharArray();
    // 返回字符数组的第一个字符
    return ret[0];
  }

  /**
   * 获取一个固定长度的随机中文字符串. <p>获取一个长度为length的随机中文字符串，该函数不能保证该字符串拥有某种语义。
   *
  ...
  /**
   * 获取一个随机长度的随机中文字符串. 得到一个长度结余minLength和maxLength之间的随机长度字符串.
   *
