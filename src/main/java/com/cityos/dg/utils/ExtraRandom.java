...
public class ExtraRandom {
  ...
  /**
   * 随机生成字符串. <p>生成一个长度为length的随机字符串，字符串中所含字符的类型由pattern和strTable共同决定。<br> <p>pattern主要包括以下几种：<br>
   * digit:生成仅由数字组成的字符串<br> letter:生成仅由字母组成的字符串<br> upper_case:生成仅由大写字母组成的字符串<br>
   * lower_case:生成仅由小写字母组成的字符串<br> digit_letter:生成由数字字母组成的字符串<br> ASCII:生成由ASCII字符组成的字符串，不允许使用strTable参数<br>
   * extend_ascii:生成由扩展ASCII字符组成的字符串，不允许使用strTable参数<br> printable_ascii:生成由可打印ASCII字符组成的字符串，不允许使用strTable参数<br>
   * chinese:生成由中文字符组成的字符串，不允许使用strTable参数<br> <p>strTable为null时，仅使用patter生成的字符表，strTable不为null时，字符表为pattern生成的字符表和strTable合并之后的结果。<br>
   *
   ...
   */
  public static String nextString(int length, String pattern, String strTable) {
    ...
    if (pattern.equals("digit")) {
      ...
      // 字符串仅由大写字母构成
      strTable = "ABCDEFGHIGKLMNOPQRSTUVWXYZ";
    } else if (pattern.equals("lowercase")) {
      // 字符串仅由小写字母构成
      strTable = "abcdefghijkmnpqrstuvwxyz";
    } else if (pattern.equals("digit_letter")) {
      // 字符串由数字或字母构成
      strTable = "abcdefghijkmnpqrstuvwxyz"
          + "ABCDEFGHIGKLMNOPQRSTUVWXYZ"
          + "1234567890";
  ...
  /**
   * 生成随机字符串. <p>生成一个长度为length的随机字符串，字符串中所含字符的类型由pattern决定。<br>
   *
   * @param length 生成字符串的长度.
   * @param pattern 字符组成表的类型.
   * @return 返回生成的随机字符串，如果参数错误，则返回null。
   * @see {@link ExtraRandom#nextString(int length, String pattern, String strTable)}
   */
  public static String nextString(int length, String pattern) {
    return nextString(length, pattern, null);
  }

  /**
   * 生成随机长度的随机字符串. <p>生成一个长度为介于minLength和maxLength之间的随机字符串，字符串中所含字符的类型由pattern决定。<br>
   *
   ...
   * @see {@link ExtraRandom#nextString(int length, String pattern, String strTable)}
   */
  public static String nextRandomLengthString(int minLength, int maxLength, String pattern)
      throws Exception {
    String str;
